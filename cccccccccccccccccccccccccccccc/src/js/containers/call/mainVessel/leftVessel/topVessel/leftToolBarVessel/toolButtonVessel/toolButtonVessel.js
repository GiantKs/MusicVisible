...
class ToolButtonVesselSmart extends React.Component{
  ...
  /*      if(TkConstant.hasRole.roleStudent){
    ...
    handlerInitAppPermissions(){
        ...
        for(let item of that.state.toolButtonDescriptionArry ){
            if(item.id === 'tool_user_list'){
                ...
                item.show = TkGlobal.isBroadcast?false:CoreController.handler.getAppPermissions('loadMedialist') ;
                item.open = false;
    ...
    getToolButtonDescriptionArry(){
        const that = this ;
        const role = ServiceRoom.getTkRoom().getMySelf().role;
        const toolButtonDescriptionArry = [
            {
                ...
                'open':false ,
                'show':(role == TkConstant.hasRole.roleChairman || role == TkConstant.hasRole.roleTeachingAssistant)?true:false,
            } , {
