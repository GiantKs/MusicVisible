/**
 * @method 获取/重置Window的requestAnimationFrame/cancelAnimationFrame
 */
((() => {
...
class sChart {
    ...
    drawPoint() {
        ...
        for (var i = 0; i < this.yEqual; i++) {
            const y = this.yFictitious * (i + 1);
            const ylen = this.yLength * (i + 1);
            this.ctx.beginPath();
            this.ctx.strokeStyle = this.axisColor;
    ...
    getYFictitious(data) {
        ...
        pow = pow > 2 ? 2 : pow;
        return Math.ceil(len / (10 ** pow)) * (10 ** pow);
    }
