...
class UserListSmart extends React.Component{
    ...
    componentDidMount() { //在完成首次渲染之前调用，此时仍可以修改组件的state
        ...
        //eventObjectDefine.CoreController.addEventListener( TkConstant.EVENTTYPE.RoomEvent.streamFailed_video , that.handlerStreamFailed_video.bind(that) , that.listernerBackupid); //streamFailed_video事件
    ...
    handlerRoomParticipantLeave(roomParticipantLeaveEventData){
        ...
        Log.error('有人走了',roomParticipantLeaveEventData)
    ...
    handlerRoomUserpropertyChanged(roomUserpropertyChangedEventData){
        ...
        for( let key of Object.keys(changePropertyJson) ){
            if( key !== 'giftnumber' ){
                if (CoreController.handler.getAppPermissions('pairOfManyIsShow')) {
                    if (key === 'publishstate' && changePropertyJson[key] !== 0) {
                        let userId = user.id;
    ...
    handlerRoomMsgList(recvEventData){ 
        ...
        for(let i=0;i<pubmsgData.length;i++){
            ...
            if(data.name === "ClassBegin"){
                if(users.role === TkConstant.role.roleTeachingAssistant || users.role === TkConstant.role.roleChairman){
                    if(TkGlobal.isBroadcast){
                        that.getOnlineUserListToPHP();
                    }
    ...
    handlerRoomPubmsg(recvEventData){
        ...
        switch(pubmsgData.name) {
            case "StreamFailure"://wj改8-10
                ...
                let user =  ServiceRoom.getTkRoom().getUsers()[userid] ;
                if(!user){ L.Logger.error('user is not exist , userid is '+userid+'!'); return;}
                if(user.initiativePublish){
                    ServiceTooltip.showPrompt(user.nickname+TkGlobal.language.languageData.alertWin.call.prompt.releaseFailed.text);
                }
                user.initiativePublish = false ;
				user.passivityPublish = false ;
                break;
            case 'ClassBegin':
    ...
    handlerRoomDelmsg(recvEventData){
        ...
        switch(pubmsgData.name)
        {
			...
			case "ClassBegin":{
                ...
                // that._productionDefaultUserlistData();
                break;
			}
        }
    }
    ...
    getOnlineUserListToPHP(){
        ...
        ServiceRoom.getTkRoom().getOnlineUser(data,function(code,response){
            if(parseInt(code) === 0){
                if(Array.isArray(response.onlineuser)){
                    // Log.debug('PHP记录',response)
                    response.onlineuser.forEach( (val, idx)=>{
                        let user = {
                            id: val.buddyid,
                            role: parseInt(val.userrole),
    ...
    addUserToList(user){
        const that = this ;
        if(TkGlobal.isBroadcast){
            if(user.role === TkConstant.role.roleAudit || user.role === TkConstant.role.roleTeachingAssistant ||  user.role === TkConstant.role.roleChairman) {
                if( !that.state.userlistData.userListItemJson.has(user.id) ){
                    ...
                    that.state.userlistData.userListItemJson = that.sortMapByRole(that.state.userlistData.userListItemJson)
                    that.setState({userlistData:that.state.userlistData});
                }
            }
        } else if(user.role === TkConstant.role.roleStudent  || user.role === TkConstant.role.roleTeachingAssistant ||  user.role === TkConstant.role.roleChairman) {
            if( !that.state.userlistData.userListItemJson.has(user.id) ){
    ...
    sortMapByRole(m){
        return new Map([...m].sort((a,b) => {return a[1]['role']-b[1]['role']}))
    }

    //排序方法开始
    sortMap(mapObj){
        let that = this;
        let values = mapObj.values();
        let arr = [];
        let result = [];
        ...
        for(let j = 1; j< 5; j++){
            result = result.concat( that.mySort(arr, j) )
        }
        result.forEach((item,index)=>{
            resultMap.set(item.value.id, item.value)
        })
        return resultMap;
    }
    mySort(arr, num){
    ...
    _productionUserItemDescInfo(user , children ){
        ...
        //    return ;
        //}
        // Log.debug('_productionUserItemDescInfo--user===>',user)
        ...
        const userItemDescInfo =  {
            ...
            onClick:TkGlobal.isBroadcast?undefined:!disabled?that.handlerUserListItemOnClick.bind(that , user.id ):undefined ,
            ...
            beforeIconArray:TkGlobal.isBroadcast?undefined:[
                {
                    show:true ,
            ...
            afterIconArray:TkGlobal.isBroadcast?undefined:[
                {
        ...
        //  wyw 2018-01-18  因为花名册不显示助教所以删除判断，但不知为何这样判断所以注释原句
        if( !CoreController.handler.getAppPermissions('showUserlistIcon') ){ //没有音视频设备则不显示用户列表的状态图标   //2017-09-18 xgd
            if(TkGlobal.isBroadcast) //直播直接返回
                return;
