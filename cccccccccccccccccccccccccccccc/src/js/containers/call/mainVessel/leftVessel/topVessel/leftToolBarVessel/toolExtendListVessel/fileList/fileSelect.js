    ...
    handlerUploadFile(){
        ...
        $("#"+that.uploadElementId ).on("change" ,function(){
            ...
            if(fileSize > MAXFILESIZE) {
                ServiceTooltip.showError(TkGlobal.language.languageData.alertWin.call.fun.uploadCourseFile.fileSizeError.text.one , false);
                return;
            }
            //上传文档参数
    ...
    render(){
        ...
        return (
                <form  id={this.uploadFormId}   style={{display:'none'}} encType="multipart/form-data"  >
                    ...
                    <input id={this.uploadElementId} type="file"  name={"filedata"} accept={(TkGlobal.isClient || TkGlobal.osType==='Mac')?'*':accept}/> {/*TODO TkGlobal.osType==='Mac' 条件是因为ios没给isClient */}
                </form>
        )
    };
};
