/**
 * 检测设备-开始检测的Smart组件
 * @module StartDetectionDeviceSmart
...
class StartDetectionDeviceSmart extends React.Component{
    constructor(props){
        super(props);
        this.state = {
            show:false
    ...
    componentDidMount() { //在完成首次渲染之前调用，此时仍可以修改组件的state
        ...
        eventObjectDefine.CoreController.addEventListener( "startDetectionDevice" , that.handlerStartDetectionDevice.bind(that) );
    };
    componentWillUnmount() { //组件被移除之前被调用，可以用于做一些清理工作，在componentDidMount方法中添加的所有任务都需要在该方法中撤销，比如创建的定时器或添加的事件监听器
        let that = this;
        eventObjectDefine.CoreController.removeAllEventListener("startDetectionDevice" );
    };

    handlerStartDetectionDevice(recvEventData){
...
export  default  StartDetectionDeviceSmart ;
