...
class H5Document extends React.Component{
            ...
            if(h5DocumentData.isH5Document){
                ...
                const callbackHandler = (fileInfo) => {//tkpc2.0.8
                    if(!open){
                        ...
                        if (source==='room-pubmsg' &&  h5DocumentData.filedata.fileid === that.fileid  &&  Number(h5DocumentData.filedata.currpage) === fileInfo.currpage) {
                            isReturn = true ;
                        }
                    }
                };
    ...
    render() {
        ...
        let styleClass = Object.assign({display:fileTypeMark == 'h5document' ? "block": "none"} , this.props.styleJson) ;
        return (
