        ...
        if (isEnter || !TkConstant.hasRole.roleTeachingAssistant) {
            ...
            ServiceRoom.getTkRoom().requestServerList(TkConstant.SERVICEINFO.hostname,TkConstant.SERVICEINFO.port,(serverlist,res)=>{
                ...
                if(serverlist && typeof serverlist === 'object'){
                    ...
                    //serverlistCopy = Object.customAssign({} , serverlist);
                }else{
        ...
        for(let [deviceKind,deviceDescJson] of Object.entries(devicesDesc) ){
            for(let  deviceDesc of Object.values(deviceDescJson) ){
                ...
                devicesElementInfo[deviceKind+'ElementArray'].push(
                    <option key={deviceKind+'_'+deviceId}  tkcustomdatadeviceid={deviceId}  tkcustomdatagroupid={groupId}  tkcustomdatakind={kind}  tkcustomdatalabel={label}  value={deviceId}  > {label}</option>
                );
            }
        }
        ...
        if(step!=undefined){
            switch (step){
                ...
                case 5:
                    ...
                    HandlerDetectionDevice.stream = null ;
                ...
                <div className="testing-bot">
                    <div className="testing-left en-testing-left">
                        <ul id="testing_box" className="testing-box">
                            ...
                            <li className={"video-pic testing-option" + (selectShow.videoinput?' active':'')  } onClick={that._cutDetectionItem.bind(that,isEnter , 2 , 'videoinput')}>
                            ...
                            <li className={"listen-pic testing-option"+ (selectShow.audiooutput?' active':'')  } onClick={that._cutDetectionItem.bind(that,isEnter,3, 'audiooutput')}>
                            ...
                            <li className={"speak-pic testing-option"+ (selectShow.audioinput?' active':'')  } onClick={that._cutDetectionItem.bind(that,isEnter,4, 'audioinput')}>
                        ...
                        <div className="video-right fixldy" id="skip-video" style={{ display:selectShow.videoinput?'block':'none' }} >
                            <div className="video-right-inside device-right-inside">
                                ...
                                <div className="camera-black" id="camera-black">
                            		{/* xueln 添加 */
                            			this.state.isClient?<embed id="videoinput_video_stream"  type="application/x-ppapi-proxy"  tkcustomdatanotdevice={that.selectDevice.videoinput===undefined?"true":undefined} ></embed>:<video id="videoinput_video_stream" autoPlay tkcustomdatanotdevice={that.selectDevice.videoinput===undefined?"true":undefined} ></video>
                            		}
                                </div>
