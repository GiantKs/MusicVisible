...
*/
    ...
    _getStudentBlackboard(){
        ...
        for(let [key,user] of Object.entries( ServiceRoom.getTkRoom().getSpecifyRoleList(TkConstant.role.roleStudent) ) ){
            if(user.playbackLeaved){continue ;} ; //|| user.publishstate === TkConstant.PUBLISHSTATE.PUBLISH_STATE_NONE
                ...
                tabContentBlackBoardElementArray.push(
                    <li className="tab-content-option "  key={value.instanceId} style={{display:value.show?'block':'none'}} >
                        <BlackboardSmart containerWidthAndHeight={this.state.containerWidthAndHeight} resizeWhiteboardSizeCallback={this.resizeWhiteboardSizeCallback.bind(this)} instanceId={value.instanceId}
                             ...
                             imageThumbnailTipContent={TkGlobal.language.languageData.header.tool.blackBoard.content.blackboardHeadTitle} showRemoteRemindContent={false}
                        />
                    </li>
                );
                            ...
                            <li className="blackboard-tool-option colors  add-position-relative" >
                                ...
                                <div className="blackboard-color-size-box">
                                    ...
                                    <ul className="tool-size-container" id="blackboard_tool_size">
                                        <li id="blackboard_size_small" onClick={that._changeStrokeSizeClick.bind(that , 'blackboard_size_small' , {pencil:5 , text:30 , eraser:30 } ) } className={"size-small "+(this.state.selectBlackboardToolSizeId === 'blackboard_size_small'?'active':'')}  >
                                            <span></span>
                                        </li>
                                        <li id="blackboard_size_middle" onClick={that._changeStrokeSizeClick.bind(that , 'blackboard_size_middle' , {pencil:15 , text:36 , eraser:90 } ) } className={"size-middle "+(this.state.selectBlackboardToolSizeId === 'blackboard_size_middle'?'active':'') }  >
                                            <span></span>
                                        </li>
                                        <li id="blackboard_size_big" onClick={that._changeStrokeSizeClick.bind(that , 'blackboard_size_big' , {pencil:30 , text:72 , eraser:150 } ) } className={"size-big "+(this.state.selectBlackboardToolSizeId === 'blackboard_size_big'?'active':'')}  >
                                            <span></span>
                                        </li>
                                    </ul>
