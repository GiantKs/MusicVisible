...
class NewpptSmart extends React.Component{
    ...
    componentDidMount() { //在完成首次渲染之前调用，此时仍可以修改组件的state
        ...
        eventObjectDefine.CoreController.addEventListener("volumeChangeDispatch" ,that.volumeChangeDispatch.bind(that) , that.listernerBackupid); //音量改变的事件
    	...
    	eventObjectDefine.CoreController.addEventListener('receive-msglist-PptVolumeControl' , that.handlerReceiveMsglistPptVolumeControl.bind(that)  , that.listernerBackupid); // ppt音量
        ...
        $(document).on("updateSlidesCountToLcElement" , function (event , data) {
            if(data){
                const callbackHandler = (fileInfo) => {
                    Object.customAssign(fileInfo , data);
        ...
        $(document).on("newppt_changeFileElementProperty" , function(event , data){
            const callbackHandler = (fileInfo) => {
                Object.customAssign(fileInfo , data); //将动态ppt数据的数据浅拷贝到文件信息中
                eventObjectDefine.CoreController.dispatchEvent({type:'setFileDataToLcElement' , message:{filedata:fileInfo} }) ;
            ...
            if(ServiceRoom.getTkRoom().getMySelf().id === user.id){
                ...
                for(let key of Object.keys(changePropertyJson) ) {
                    if(key === 'volume' ) { //发布状态改变时显示或者隐藏video
                        //处理ppt音量的改变
                        this.setState({volumeValue:changePropertyJson[key] / 100});
                        let data = {
                            action:"PptVolumeControl" ,
                            volumeValue:changePropertyJson[key] / 100
                        };
                        ...
                        this._postMessage(data);
                        objtrack.style.height=changePropertyJson[key]+"%";
                        objhandle.style.bottom=changePropertyJson[key]+"%";
                    }
                }
            }
        // }
    ...
    handlermobileSdk_changeDynamicPptSize(recvEventData){
        if(TK.SDKTYPE === 'mobile' &&  recvEventData.message && typeof  recvEventData.message === 'object'){
            ...
            Object.customAssign(data , recvEventData.message) ;
            this._postMessage(data);
        }
    };
    ...
    recvEventDataPptVolumeControl(pubmsgData){
    	let that = this ;
    	that.state.volumeValue = pubmsgData.data.volume;
    	that.setState({volumeValue:that.state.volumeValue})
    	let data = {
            action:"PptVolumeControl" ,
            volumeValue:pubmsgData.data.volume
        };
        this._postMessage(data);
    }
    ...
    _openAynamicPPTHandler(filedata){ //打开动态PPT
        ...
        let options = {
          // rPathAddress:   "https://192.168.1.251:8443" + swfpath+"/"  ,
            ...
            PresAddress:"newppt.html?remoteHost="+window.location.host+"&remoteProtocol="+window.location.protocol +"&versions="+newpptVersions+"&fileid="+fileid+"&playback="+TkGlobal.playback
                ...
                +"&dynamicPptDebug="+TkConstant.DEV+"&PptVolumeValue=" + that.state.volumeValue +"&ts="+new Date().getTime() ,
            slideIndex:pptslide ,
            stepIndex:pptstep ,
            fileid:fileid ,      
        };
    ...
    _newpptAutoPlayVideoInNewPpt(pptVideoJson){ /*动态PPT视频的处理函数*/
        ...
        if(TK.SDKTYPE === 'mobile'){
            ...
            let mediaAttributes = { filename:"" , fileid:pptVideoJson.fileid , type:'media' , pauseWhenOver: pptVideoJson.isvideo && TkConstant.joinRoomInfo.pauseWhenOver } ;
	...
	/*老师本地改变音量*/
	volumeChangeDispatch(newpptData){
		const that = this ;
		that.state.volumeValue = newpptData.message.data.volume;
		that.setState({volumeValue:that.state.volumeValue});
		let data = {
	        action:"PptVolumeControl" ,
	        volumeValue:that.state.volumeValue
	    };
	    this._postMessage(data);
	}
    render(){
        ...
        let styleClass = Object.customAssign({display:fileTypeMark !== 'dynamicPPT' ? "none": "block"} , this.props.styleJson) ;
        return (
