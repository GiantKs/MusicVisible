...
class AnswerStudentToolSmart extends React.Component {
	...
	_loadTimeDescArray(desc){
			 let beforeArray = [] ;
			  desc.map((item,index) => {		
				let a = <li className="answer-teach-lis" key={item.id} style={{background:desc[index].sel? "#2196f3" : undefined , cursor: this.state.lisStyle?"default":"pointer"}}   onClick={this.changeColor.bind(this,index)}>{item.name}</li>;//tkpc2.0.8
				beforeArray.push(a)
			});
			return{
