...
class ToolExtendListVesselSmart extends React.Component{
    ...
    componentDidUpdate(prevProps, prevState) { //每次render结束后会触发
      /*  if( this.nowOpen !== this.oldOpen){
            this.oldOpen = this.nowOpen ;
            if(this.props.changeZIndex && typeof this.props.changeZIndex === 'function' ){
                this.props.changeZIndexFromIsOpen(this.nowOpen);
            }
        }*/
    }
    ...
    render(){
        let that = this ;
        if( CoreController.handler.getAppPermissions('toolListIsShowPairMany') ){
            this.nowOpen = true ;
        }else{
            this.nowOpen = false ;
        ...
        let listWidth = CoreController.handler.getAppPermissions('toolListIsShowPairMany')?'2.8rem':'3.6rem' ;
        let tool_extend_container_style = {
            // left:CoreController.handler.getAppPermissions('toolListIsShowPairMany')?'100%':(this.nowOpen?'100%':undefined),
            width:!this.nowOpen?'0rem':listWidth,
        };
        return (
            <article className="tool-extend-container tk-weight" id="tool_extend_container"  style={  Object.customAssign( {} , this.props.styleJson , tool_extend_container_style )} >
                {(TkConstant.joinRoomInfo.isClassOverNotLeave || that.state.listLoad.tool_courseware_list)?<FileListSmart styleJson={{width:listWidth}}  show={that.state.listShow.tool_courseware_list} isUploadH5Document={this.state.isUploadH5Document} isMediaUI={false}  idType={that.state.tool_common_type}/>:undefined} {/* 普通文件列表*/}
                {that.state.listLoad.tool_media_courseware_list?<FileListSmart  styleJson={{width:listWidth}} show={that.state.listShow.tool_media_courseware_list} isMediaUI={true}   idType={that.state.tool_media_type}/>:undefined} {/* 媒体文件列表*/}
                {that.state.listLoad.tool_user_list?<UserListSmart styleJson={{width:listWidth}} show={that.state.listShow.tool_user_list} /> :undefined} {/* 用户列表*/}
            </article>
        )
    };
