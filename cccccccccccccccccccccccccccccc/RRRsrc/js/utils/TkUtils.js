...
const tkUtils  = {
    ...
    * @method filterContainDataAttribute */
    ...
    getPagingToolLT:function(that,percentLeft,percentTop,id) {//获取白板区工具相对白板的位置
        //获取拖拽的元素宽高：
        ...
        if (dragEle) {
            ...
            if (dragEleW !== 0) {
                const defalutFontSize = window.GLOBAL.windowInnerWidth / TkConstant.STANDARDSIZE ;
                //获取白板区域宽高：
    ...
    /*低级比较两个json是否值一样*/
    lowclassCompareJsonIsCommon:(json1 , json2)=>{
        if(json1 === json2){
            return true ;
        }else{
            for(let [key,value] of Object.entries(json2)){
                if(json1[key] !== value){
                    return false ;
                }
            }
    ...
    /*去掉rem，转成数字*/
    replaceRemToNumber:(remStr) => {
        if(remStr && typeof remStr === 'string'){
            remStr = Number( remStr.replace(/rem/g , "") );
        }
        return remStr ;
    },
    /*去掉calc()*/
    replaceCalc(calcStr){
        let a = calcStr ;
        if(calcStr && typeof calcStr === 'string'){
            calcStr = calcStr.replace(/(^\s*)|(\s*$)/g, "");
            calcStr =  calcStr.replace(/(^calc\s*\(|\)$)/g , "");
        }
        return calcStr ;
    } ,
    /**
